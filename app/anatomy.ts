export type SystemId = 'skeletal'|'muscular'|'arterial'|'venous'|'nervous'|'digestive'|'respiratory'|'urinary'|'reproductive'|'lymphatic'|'endocrine'|'integumentary'|'connective'|'sensory'|'cardiac'|'breast';
export const SYSTEMS: {id:SystemId;name:string;color:string;description:string}[] = [
 {id:'breast',name:'Breast anatomy',color:'#bd8d91',description:'Female reference meshes show mammary lobes, ducts, sinuses, adipose tissue, nipples, areolae, and suspensory ligaments.'},
 ...
 {id:'reproductive',name:'Reproductive',color:'#bda098',description:'The ovaries, uterine tubes, uterus, cervix, and vagina are shown using fitted female reference meshes. This layer also includes supporting ligaments and clearly labelled schematics of the vulva and uterine wall layers.'},
...
export interface Part {description?:string;id:string;name:string;conceptId:string;sourceId?:string;sourceUrl?:string;provenance?:'adapted'|'female-reference'|'schematic';system:SystemId;chunk:number;positions:number;normals:number;indices:number;vertexCount:number;indexCount:number;bounds:[number[],number[]]}
export interface Concept {description?:string;id:string;name:string;sourceId?:string;elements:string[]}
export interface Atlas {version:string;sex?:'female';source?:string;scope?:string;parts:Part[];concepts:Concept[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];triangles:number}
export type View = 'three-quarter'|'front'|'back'|'side';
export interface SceneState {inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number}
export const DEFAULT_VISIBLE:SystemId[] = ['cardiac','sensory','skeletal','muscular','arterial','venous','nervous','respiratory','digestive','urinary','lymphatic','endocrine','reproductive','connective','breast'];
export const EXPLANATIONS:Record<string,string> = {
 'uterus':'A muscular pelvic organ that connects to the vagina through the cervix.',
 'ovaries':'Paired reproductive organs that produce egg cells and hormones.',
 'fallopian tubes':'Paired uterine tubes extending from the uterus toward the ovaries.',
 'breasts':'Mammary glands and surrounding tissues, including their lobes, ducts, and supporting structures.',
 'female pelvis':'The fitted female pelvis includes the sacrum, coccyx, and the ilium, ischium, and pubis on both sides.',
 'female urethra (schematic)':'A simplified tube illustrating the route from the bladder neck toward the exterior, anterior to the vaginal opening. Its geometry is authored for this study model.',
